...
pacman::p_load(mosaic, psych, corrplot, ggbiplot, REdaS)
pacman::p_load(here)
```
...
Da nun klar ist, dass sich die Daten für eine Hauptkomponentenanalyse eignen, wird im Anschluss die Anzahl der Hauptkomponenten bestimmt, welche ausreichend sind, um die wesentliche Struktur der betrachteten Merkmale abzubilden. Dies geschieht unter anderem anhand der Eigenwerte unserer Datenmatrix.

[*Exkurs: Was sind Eigenwerte?*]{.underline} *\
Multipliziert man eine Matrix mit einem Vektor, erhält man als Ergebnis wieder einen Vektor. Für quadratische Matrizen existieren bestimmte Vektoren - sogenannte **Eigenvektoren** -, die, wenn sie mit der Matrix multipliziert werden, ein Vielfaches des ursprünglichen Vektors ergeben. D. h. der Eigenvektor ändert seine Richtung nicht, er wird also nur skaliert. Den Skalierungsfaktor bezeichnet man als **Eigenwert** einer Matrix.*
...
```{r}
# Screeplot
scree(irll_norm, factors = FALSE, hline = -1)
```
...
Dazu kann der vorherige Code so ergänzt werden, dass eine Linie bei y = 1 in den Plot gezeichnet. Man kann sich die Eigenwerte aber auch über den Befehl `eigen(cor_mtx)$value` ausgeben lassen.

```{r}
#Screeplot mit Linie bei y=1
...
Um die Ausgabe etwas übersichtlicher zu gestalten, wird anschließend an die Analyse noch ein nicht benötigter Teil der Ausgabe gelöscht. Die Ladungen (Korrelationen der ursprünglichen Variablen mit den Hauptkomponenten) werden außerdem nur ab einem Betrag von 0.5 angezeigt, gerundet und sortiert (jeweils innerhalb einer Komponente nach absteigenden Beträgen der Ladungen. Zusätzlich wird weiter unten noch einmal der Anteil der Hauptkomponenten an der Gesamtvarianz, sowie die kumulierte Varianz angegeben.
...
Schaut man sich die Ladungen der einzelnen Variablen auf die Hauptkomponenten an, so erkennt man, dass einige Variablen auf mehrere Hauptkomponenten hoch laden. Dieser Umstand macht eine inhaltliche Interpretation der einzelnen Hauptkomponenten schwer.

## Rotation

Um die Hauptkomponenten inhaltlich besser interpretieren zu können, wird eine Rotation der Hauptkomponenten vorgenommen. Das Ziel der Rotation ist, dass auf jeder Hauptkomponente einige Variablen hoch und die übrigen Variablen möglichst niedrig laden. Zusätzlich sollte jede Variable nur auf einer einzelnen Hauptkomponente hoch laden, auf den übrigen niedrig.
