...
```{r}
# Histogramm
gf_histogram( ~ Entfernung, data = Umfrage, center = 0.5, binwidth = 1) #2,4?
```
...
-   Wie würden Sie die Verteilung beschreiben?
    ...
    -   *Antwort: Eine rechtsschiefe Verteilung.*
...
-   Erhöht Re-Sampling den ursprünglichen Stichprobenumfang?
    ...
    -   *Nein, beim Bootstraping wird die Anzahl der Beobachtungen nicht verändert.*
...
-   Welche Verteilungsform zeigt das Histogramm des Mittelwertes der Entfernung in den Re-Samples?
    ...
    -   *Antwort: Es zeigt eine Normalverteilung.*
...
-   Stimmt die Aussage: Mit einer Wahrscheinlichkeit von 95% wird der Wert einer Beobachtung $x_i$ zwischen `r qdata( ~ mean, data = Bootvtlg, p = 0.025)` und `r qdata( ~ mean, data = Bootvtlg, p = 0.975)` liegen?
    ...
    -   *Antwort: Nein. 1. Es müsste "Mittelwert einer Bootstrap-Stichprobe* $\mu_i$*" und nicht "Wert einer Beobachtung* $x_i$*" heißen. 2. Die frequentistisch korrekte Interpretation eines KIs lautet, dass der Mittelwert einer Bootstrap-Stichprobe* $\mu_i$ *in 95% der Fälle in den Grenzen des KIs liegt.*
