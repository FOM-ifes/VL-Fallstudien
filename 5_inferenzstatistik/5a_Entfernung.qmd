...
Zu Beginn des Sommersemesters 2022 wurden in fünf Kursen Studierende an zwei Standorten der FOM in einem freiwilligen, anonymen Online-Fragebogen nach Ihrem Fahrtweg zum Studienzentrum gefragt.

Die genaue Frage lautet:

> Wie weit (gerundet auf ganze km) ist Ihr üblicher Fahrtweg zum Studienzentrum?

Eine zufällige Stichprobe der Antworten darauf liegt in der Datei `results-survey592526.csv` vor.
...
- Welchen Wert würden Sie als **Punktschätzer** $\hat{\mu}$ verwenden?

- Wird bei einer erneuten Stichprobe aus derselben Zielpopulation das Ergebnis wieder  $\bar{y}=`r round(mw_Umfrage, 2)`$ lauten?
...
```{r}
Bootvtlg <- do(10000) * mean( ~ Entfernung, 
                              data = resample(Umfrage))
# Obere Beobachtungen
head(Bootvtlg)
```
