...
Sei $\pi$ der Anteil der Fälle aus der Impfgruppe, dann kann die Formel der Impfwirksamkeit wie folgt dargestellt werden (wenn Impf- und Placebogruppe gleich groß sind):
...
- Wie groß ist hier der Punktschätzer $\hat{\pi}$ für die unbekannte Wahrscheinlichkeit $\pi$?
...
Der Befehl `rflip()` simuliert den Münzwurf. Hier werden jetzt $10000$ Simulation der Anzahl Geimpfter unter den $170$ Fällen mit $\pi_0 =0.5$ durchgeführt:

```{r}
Nullvtlg <- do(10000) * rflip(n = 170)
```

In der Variable (Spalte) `heads` der Datentabelle `Nullvtlg` steht die zufällige Anzahl Geimpfter unter den Fällen der Simulationen (Zeile) von Daten, wenn die Nullhypothese stimmen würde.
