...
- Wie stark verringert sich das Risiko einer Erkrankung durch die Impfung?
  ...
  *Antwort:*\
  Das ist das relative Risiko, also
  
  $$\frac{rk_{Impfung}}{rk_{Placebo}}.$$
...
- Wie hoch ist die Impfwirksamkeit?
  ...
  *Antwort:* Die Wirksamkeit einer Impfung ist definiert als
  ...
  $$IW=1-\frac{rk_{Impfung}}{rk_{Placebo}}.$$

```{r}
#| label: Impfwirksamkeit
iw <- 1 - (rk_impf / rk_placebo)
iw
```
...
Sei $\pi$ der Anteil der Fälle (Erkrankungen), die aus der Impfgruppe stammen, dann kann die Formel der Impfwirksamkeit wie folgt dargestellt werden (wenn Impf- und Placebogruppe gleich groß sind):
...
- Was ist unsere Forschungshypothese?
  ...
  *Antwort:*
  ...
  Uns interessiert, ob die Impfung wirkt, dass ist die Forschungshypothese.
...
- Wie groß wäre $\pi$, wenn der Impfstoff wirken würde?
  ...
  *Antwort:* Wenn der Impfstoff vollständig vor einer Erkrankung schützen würde, dann kämen weniger als die Hälfte der Fälle aus der Impfgruppe, also $\pi < 0.5$.
...
$$Pr(Y=y |\pi, n) = \binom{n}{y} \cdot \pi \cdot (1-\pi)^{n-y}.$$
