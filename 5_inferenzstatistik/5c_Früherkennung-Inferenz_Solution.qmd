...
-   Wie hoch ist die Wahrscheinlichkeit, bei einem positiven
    ...
    e.  ca. 30%
    f.  ca. 15%
    g.  ***ca. 7.5%***, wir können es selber ausrechnen.
...
Der **Satz von Bayes**[^4] lautet hier:
...
aber nur $8$ korrekt sind[^5], also

[^5]: Das ist die Präzision (englisch: *precision*).
...
Es gibt in der Zielpopulation der FOM-Studierenden dieses Studienganges
einen unbekannten Anteil $\pi$, der die obige Frage richtig beantworten
kann.

-   Was schätzen Sie? Wie groß ist dieser Anteil?
-   Und wie sicher sind Sie sich, dass Ihre Antwort richtig ist?

Nutzen Sie zur Beantwortung dieser Frage die App
<https://fomshinyapps.shinyapps.io/BaBeBi/> ***(Anteil und Sicherheit
einstellen)*** und notieren Sie die ***bei der Betabinomialfunktion***
ausgegebenen Werte für $\alpha$ und $\beta$. Wenn für Sie z.$\,$B. alle
Werte für $\pi$ gleich plausibel sind, dann gilt $\alpha=\beta=1$:

*Hinweis*: Erklärung und Auflösung folgen in der Fallstudie zur
...
Geben wir das Ergebnis unseres Kurses ein:[^6]

[^6]: *Hinweis*: In der Vorlage steht `n <- 40` und `y <- 15` . Diese
...
-   Nur mit diesen Daten, der Stichprobe: Was würden Sie tippen, wie
    ...
    unten fehlenden Wert (`NA`) für `pi_dach`:
...
*Hinweis:* $\hat\pi$ ist der Punktschätzer für den unbekannten Anteil
$\pi$ in der Population.

------------------------------------------------------------------------

Unter der Bedingung, dass für jeden unser $n$ Versuche die gleiche
Erfolgswahrscheinlichkeit $\pi$ vorliegt, kann die Wahrscheinlichkeit für
...
Je nachdem wie groß $\pi$ ist, desto *mutmaßlicher* (engl.: *likely*)
ist unser Ergebnis:
...
Ein guter Punktschätzer (Maximum der Likelihoodfunktion) für den Wert
des Parameters $\pi$ in der Zielpopulation ist also der Anteilwert $p$
in unser Stichprobe:
...
zufälliges Resample - mit Zurücklegen.

*Hinweis*: Die folgende Herangehensweise dient der Illustration. Für das
Vorgehen sollte $n>35$ sein. Sollte $n$ kleiner sein, ist die Lösung
*unter theoretischer Verteilungsannahme* (s. u.) zu bevorzugen.
...
Simulieren wir den Vorgang des zufälligen Resampling 10000-mal und
...
in den Resamples aus der Datentabelle `Bootvtlg` kann über ein
...
-   Wie ändert sich der Standardfehler, wenn mehr Bootstrap-Stichproben
    gezogen werden?
...
-   Stimmt die Aussage: Mit einer Wahrscheinlichkeit von 95% wird der
    ...
    *Antwort:* Nein, da die aus vielen Stichproben geschätzten
    Stichprobenstatistiken mit einer 95%igen Sicherheit im Intervall
    liegen.
...
-   Was ist die Bedeutung des Konfidenzintervalls?
    ...
    *Antwort:*
    ...
    -   Wenn wir die Anzahl der Resamples erhöhen, ändert sich das
        Konfidenzintervall nicht systematisch: Daher sind wir uns zu 95%
        (bzw. 99%, je nach Konfidenzniveau) sicher, dass eine nächste
        Stichprobe ebenfalls eine Stichprobenstatistik im Intervall
        ergeben wird.
    -   Damit haben wir auch die Sicherheit, dass der unbekannte Wert
        des Parameters in der Population ebenfalls in diesem Intervall
        liegen wird. Das Konfidenzintervall liefert uns somit einen
        plausiblen Wertebereich für den Wert des Parameters in der
        Population.
    -   Je höher das Konfidenzniveaus ist, desto breiter ist c.p. das
        Konfidenzintervall.
    -   Je größer der Stichprobenumfang ist, desto kleiner ist c.p. der
        Standardfehler und damit das Konfidenzintervall schmaler.
