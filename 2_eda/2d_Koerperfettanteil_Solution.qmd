...
- Wie hoch ist der arithmetische Mittelwert des Körperfettanteils?
  -   Der Mittelwert beträgt `r mean(~ Koerperfettanteil, data = body_data)`.

- In welchem Bereich liegen 50% der Werte für den Körperfettanteil?
  -   Üblicherweise `Q1`-`Q3`, aber auch `min`-`median` oder `median`- `max`.
...
Am häufigsten lagen die BMI-Wert zwischen 24 und 26. Die Verteilung ist rechtsschief. Der BMI weist insgesamt eine Spannweite von 18 bis 38 auf. Es gibt einen Ausreißer mit einem BMI über 35.
...
```{r}
# Streudiagramm
gf_point(Koerperfettanteil ~ Bauchumfang, data = body_data)

# Korrelation
cor(Koerperfettanteil ~ Bauchumfang, data = body_data)
```
...
1 - pdata( ~ Bauchumfang, q = 94, data = body_data)
```
...
body_data_subset <- body_data |>
filter(Bauchumfang >= 94)

str(body_data_subset)

# Empirische Verteilungsfunktion
pdata( ~ BMI, q = 25, data = body_data_subset)
```
