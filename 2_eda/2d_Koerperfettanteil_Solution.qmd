...
- Kennen Sie eine dieser Methoden?
  ...
  *Mögliche Antworten:* Body-Mass-Index (BMI), Waist-Hip-Ratio, Bauchumfang ...

Im Folgenden wird der Zusammenhang zwischen dem Körperfettanteil und Methoden zur indirekten Schätzung des Körperfetts etwas genauer betrachtet. Dazu liegt eine Tabelle mit Gesundheitsdaten von 249 Männern vor. Für diese Fallstudie wurde die orginale Datentabelle [Body Fat Prediction Dataset](https://https://www.kaggle.com/datasets/fedesoriano/body-fat-prediction-dataset) bereinigt (Ausreißer in den erklärenden Variablen wurden entfernt). Zusätzlich wurde die Datentabelle von 15 auf 5 Spalten reduziert und ins Deutsche übersetzt. Die Spalten der Datentabelle sind:
...
- Wie hoch ist der arithmetische Mittelwert des Körperfettanteils?
  ...
  *Antwort:* Der Mittelwert beträgt `r mean(~ Koerperfettanteil, data = body_data)`.
...
```{r}
# Streudiagramm
gf_point(Gewicht ~ Groesse, data = body_data)
# ggf. ergänzt mit Glättungslinie
gf_point(Gewicht ~ Groesse, data = body_data) |>
  gf_smooth()
```
...
- Wofür steht ein Punkt in der Abbildung?
  ...
  *Antwort:* Für die Beobachtungen zweier Variablen einer Beobachtungseinheit (ein Mann).
...
- Wie lässt sich dieser Zusammenhang interpretieren?
  ...
  *Antwort:* Mit steigendem BMI wird in der Stichprobe im Mittel auch ein steigender Körperfettanteil beobachtet.
...
```{r}
gf_histogram( ~ BMI, bins = 20, data = body_data)
```
...
- Was können Sie dem Histogramm entnehmen?
  ...
  *Antwort:* Am häufigsten lagen die BMI-Werte ca. zwischen 24 und 26. Die Verteilung ist rechtsschief. Der BMI weist insgesamt eine Spannweite von ca. 18 bis 38 auf. Es gibt einen Ausreißer mit einem BMI über 35.
  
  *Hinweis:* Für Ausreißer-Erkennung ist ein Boxplot besser geeignet.
...
```{r}
pdata( ~ BMI, q = 24.9, data = body_data) - 
  pdata( ~ BMI, q = 18.5, data = body_data)
# Alternativ mit c(...) und diff()
pdata( ~ BMI, q = c(18.5, 24.9), data = body_data) |> diff()
```
...
Die Umkehrfunktion zur Verteilungsfunktion ist die Quantilsfunktion. Mit Hilfe der Quantilsfunktion (`qdata`) kann angegeben werden, welcher Wert für eine gegebene relative Häufigkeit `p` nicht überschritten wird, d. h. die jeweiligen Quantile. Z. B. gilt für 10 % der Männer, dass der BMI nicht größer als `r qdata( ~ BMI, p = 0.1, data = body_data)` war:
...
```{r}
qdata( ~ BMI, p = 0.5, data = body_data)
```
...
```{r}
gf_point(Bauchumfang ~ Koerperfettanteil, data = body_data)
```

Auch hier stehen beide Variablen in einem linearen Zusammenhang.
...
- Wie groß ist der Anteil der Männer mit einem Bauchumfang von 94 cm oder mehr? Deckt sich der Anteil mit den als vom BMI als übergewichtig eingestuften Teilnehmern?

  *Antwort:*

```{r}
# Anteil Bauchumfang - übergewichtig
tally( ~ Bauchumfang >= 94, data = body_data, format = "prop")
# Anteil BMI - übergewichtig
tally( ~ BMI >= 25, data = body_data, format = "prop")
```

Nach dem Kriterium *Bauchumfang* werden weniger Männer als übergewichtig eingestuft.
...
```{r}
# absolut
tally( ~ Bauchumfang >= 94 & Koerperfettanteil < 25, data = body_data)
# Anteile
tally( ~ Bauchumfang >= 94 & Koerperfettanteil < 25, data = body_data, format = "prop")
```

Ja, siehe die Merkmalskombinationen `TRUE`, `TRUE`.

*Ergänzung:*

Um die sowohl nach Bauchumfang und nach Körperfettanteil gleich eingestuften Männer zu identifizieren, kann die zweite Bedingung umgedreht werden.
...
Dann sind die nach beiden Einstufungen gleich klassifizierten Häufigkeiten auf der Hauptdiagonalen (von oben links nach rects unten).