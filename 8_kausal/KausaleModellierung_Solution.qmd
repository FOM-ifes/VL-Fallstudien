...
## Grundelemente am Beispiel

Als Beispiel wird der Datensatz `kreditstatus.csv`^[Die Daten stammen ursprünglich von Lending Club, wurden durch DataCamp bearbeitet und bereitgestellt und hier mit einigen weiteren Modifikationen eingesetzt. Lending Club ist ein Peer-to-Peer-Kreditvermittler, der Daten zu angenommenen und abgelehnten Krediten über viele Jahre frei veröffentlicht hat. Mittlerweile ist der Zugriff nicht mehr möglich. Eine wichtige Änderung in der Datentabelle im Vergleich zu den Originaldaten betrifft den Kreditstatus, der ursprünglich statt Ausfall (ja/nein) den Kreditstatus (laufend/zurückgezahlt/ausgefallen) gezeigt hat -- daraus wurden nur die Datensätze mit Status *zurückgezahlt* und *ausgefallen* auswählt. ] genutzt, der folgende Variablen enthält:
...
`arbeit` | Dauer des Anstellungsverhältnisses in Jahren^[Hinweis: Hier gibt es einige unplausible Werte, z.B. im Alter von 23 schon über 60 Jahre im Anstellungsverhältnis.]
...
KRED |> mutate(nbonitaet = as.integer(bonitaet)) -> KRED
# Reihenfolge der Stufen umkehren
KRED |> mutate(nbonitaet = max(nbonitaet) + 1 - nbonitaet) -> KRED
...
dagify(ausfall ~ einkommen,
  ...
  # statt |> ... geht auch + theme_dag_blank
```
...
# Koordinaten festlegen
co <- data.frame(x = c(1, 2, 3), y = c(0, 1, 0), 
...
- Bestimmen Sie alle Pfade von Bonität zu Ausfall mit dem DAG aller Variablen (siehe oben) und unterscheiden dabei nach Vorder- und welche Hintertüren.
...
- Modellieren Sie das Modell mit `glm(..., family = binomial(logit))`. *Hinweis*: Da `ausfall` eine binäre kategoriale Variable ist, wird hier die logistische Regression genutzt.
...
- Vermutlich hat sich der Koeffizient für `alter` geändert. Was ist der Grund dafür?
   ...
   Sample-Selection-Bias: Verzerrung durch Auswahl der Stichprobe nach einem Collider.
