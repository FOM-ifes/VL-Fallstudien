...
- **Priori-Verteilung** $Pr({\pi})$: Wahrscheinlichkeitsverteilung von   $\pi$, *bevor* wir unsere Daten haben.

- **Likelihood** $Pr({p} \mid {\pi})$: *Mutmaßlichkeit* von $p$ bei gegebenem $\pi$.

- **Posteriori-Verteilung** $Pr({\pi} \mid {p})$: Wahrscheinlichkeitsverteilung von $\pi$ *nachdem* wir unsere Daten haben.
...
- Welche Verteilungsform hat *Ihre* Priori-Verteilung?
  ...
  *Antwort:* Individuelle Lösung. Hängt von den gewählten Parametern (`alpha`, `beta`) ab.
...
- Berechnen Sie Modus und Erwartungswert der Posteriorverteilung.

  *Antwort:*
  
```{r}
#| label: Posteriori-Kennzahlen
...
- Berechnen Sie das 95%-Kredibilitätsintervall
...
- Sind die Kennzahlen der Posteriori-Verteilung unabhängig von der
  ...
  *Antwort:* Nein, außer bei uninformativen Prior wie Beta(1,1).
...
- Wo liegt der Erwartungswert der Posteriori-Verteilung im Verhältnis
  ...
  *Antwort:* I. d. R. liegt der Erwartungswert zwischen dem Erwartungswert der Priori-Verteilung und dem der Likelihood.
