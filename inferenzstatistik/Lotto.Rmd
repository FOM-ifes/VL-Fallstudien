---
title: "Lotto. Verganges Glück oder Pech?"
author: "Ihr Name"
date: "`r Sys.Date()`"
output:
  html_document:
...
# Vergangenes Lotto Glück?

Besuchen Sie die Seite [https://fomshinyapps.shinyapps.io/Lottotipp/](https://fomshinyapps.shinyapps.io/Lottotipp/).

Wählen Sie 6 aus 49 Zahlen aus und notieren Sie sich den Anteil der Spiele an denen Sie mindestens drei Richtige hatten, z.B. `0.0199649`:
...
Dem Abgleich *Ihres* Tipps mit den Lottozahlen von 1955 bis Ende 2021 liegen $n = 4558$ Ziehungen zugrunde.
...
- Berechnen Sie, an wie vielen Tagen Sie gejubelt hätten.
...
Beim Lotto werden 6 aus 49 Zahlen gezogen. Für die erste Kugel gibt es damit 49 Möglichkeiten. Da diese nicht zurückgelegt wird, gibt es für die nächste Kugel 48 Möglichkeiten. Für 6 Kugeln ergeben sich damit
...
Um zu bestimmen wir wahrscheinlich es ist mindestens 3 Richtige zu haben müssen die Wahrscheinlichkeiten für 3,4,5 und 6 Richtige addiert werden:
...
pi_gewinn <- 
  ...
  (choose(6, 0) * choose(43, 0)) / choose(49, 6)   # 6 Richtige, 0 Falsche
pi_gewinn
```
...
Simulieren wir $3\times$ (`do()*`) $n = `r n`$ Ziehungen mit einer Gewinnwahrscheinlichkeit von  $\pi = `r pi_gewinn`$ (`(rflip()`):
...
Auch ohne Manipulation, oder dass Sie ein besonderer Glückspilz oder Pechvogel vor, kommt es rein zufällig vor, dass der Anteil der Richtigen in einer Stichprobe variiert.
...
abweichung <- p_richtig - pi_gewinn
abweichung
...
- Ein Ergebnis wird *Signifikant zum Niveau $\alpha=5\%$ genannt, wenn der p-Wert kleiner als $\alpha$ ist. Ist Ihr Ergebnis signifikant?
