...
- Für 20-25 jährige Männer gehen wir von $\mu=181cm$ und $\sigma=7.5cm$ aus:
```{r}
...
Wenn die Daten normalverteilt sind, und wir $\mu$ und $\sigma$ kennen, können wir Wahrscheinlichkeiten bestimmen, z.B., dass eine Frau kleiner oder gleich $160cm$ ist:
...
- Ändern Sie den Code so, dass Sie die Wahrscheinlichkeit bestimmen können, dass eine Frau kleiner oder gleich $180cm$ ist:

```{r}
xpnorm(q = 160, mean = 168, sd = 6)
```

- Wie groß ist die Wahrscheinlichkeit, dass eine Frau größer als $180cm$ ist?

## Quantile bestimmen

Die Quantilsfunktion, $q=F^{-1}(p)$, gibt den Wert $q$ zurück, der mit einer Wahrscheinlichkeit $p$ nicht überschritten wird. So sind z.B. unter diesen Annahmen $90\%$ der Frauen kleiner oder gleich $`r xqnorm(p = 0.9, mean = 168, sd =6) %>% round(2)`$:
...
Z.B. hat eine $174cm$ große Frau einen z-Wert von $+1$:

$$z=\frac{x-\mu}{\sigma}=\frac{174-168}{6}=+1$$ 

D.h., ihre Größe ist eine Standardabweichung über den Mittelwert

#### Fragen

- Berechnen und interpretieren Sie den z-Wert einer Frau, die $156cm$ groß ist.

- Berechnen Sie den z-Wert Ihrer Größe in R. Für die Damen $\mu=168$ und $\sgima=6$, für die Herren $\mu=181$ und $\sigma=7.5$
...
Simulieren wir eine Datentabelle von jeweils $10000$ Frauen und Männer, mit normalverteilter Größe und $\mu_F=168, \sigma_F=6$ bzw. $\mu_M=181, \sigma_M=7.5$
...
- Angenommen vor Ihnen steht eine Person, die $185cm$ groß ist, und Sie können das Geschlecht nicht erkennne. Was schätzen Sie, welches Geschlecht hat die Person wahrscheinlich?
...
- Wären Sie sich bei einer Größe von $174.5cm$ bzgl. des Geschlechts *sicher*?
...
\begin{eqnarray*}
Pr(Geschlecht|Größe) &=&\frac{Pr(Größe|Geschlecht)\cdot Pr(Geschlecht)}{Pr(Größe)} \\
&\propto& Pr(Größe|Geschlecht)\cdot Pr(Geschlecht)
\end{eqnarray*}

Für $174.5cm$ und a priori $90\%$ weiblich ergibt sich:
