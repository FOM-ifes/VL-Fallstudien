...
- Können wir sagen, dass jemand mit einer `Sicherheit` von 8 doppelt so sicher ist wie jemand mit einer `Sicherheit` von 4?
...
- Welchen Zusammenhang vermuten Sie zwischen `Sicherheit` und `Erkannt`?

***

Über ein Streudiagramm (`gf_point(y ~ x, data = Daten)`) kann die gemeinsame Verteilung der Variablen dargestellt werden:

```{r Streudiagramm}
gf_point(Erkannt ~ Sicherheit, data = EineLuege)
```
...
- Können Punkte übereinander liegen?

***

Den Zusammenhang können wir auch modellieren:^[Die methodische Umsetzung ist hier ein Ausblick!]
